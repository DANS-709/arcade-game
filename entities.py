...
class NPC(arcade.Sprite):
    def __init__(self, npc_data, x, y, scale):
        # npc_data: dict с полями name, race, cls, stats, prompt_template
        self.name = npc_data.get("name", "NPC")
        self.race = npc_data.get("race", "human")
        self.cls = npc_data.get("class", "commoner")
        self.base_stats = npc_data.get("stats", {})
        self.prompt_template = npc_data.get("prompt_template", "")

        image = npc_data.get("image", "images/npc.png")
        try:
        ...
        self.role = "npc"
        self.reputation = 0  # отношение к игроку

    def build_prompt(self, player):
        # player: выбранный герой (Entity)
        # Формируем системный промпт для LLM
        return self.prompt_template.format(
            npc_name=self.name,
            npc_race=self.race,
            npc_class=self.cls,
            npc_stats=self.base_stats,
            npc_reputation=self.reputation,
            player_name=getattr(player, "name", "Player"),
            player_class=getattr(player, "cls", None),
            player_race=getattr(player, "race", None),
            player_stats=player.stats_dict
        )
